...
String showTemp () {
  return "<script>let elem = document.querySelector(\".temp\");elem.innerHTML = \"Temperature "+String((int)temp)+"°С\";</script>";
  }
...
void measure_temp(MillisTimer &mt){
  ...
  Serial.println("Temperature measurment ");
...
void setup() {
  ...
  light_timer.start();
   ...
   server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request) {
        ...
        String html_send = web + showTemp();
        request->send(200, "text/html", html_send+setRozklad());
   });
   server.on("/feed_fish", HTTP_GET, [] (AsyncWebServerRequest *request){
      ...
      feed_timer.setRepeats(1);
      feed_timer.start();
      String html_send = web +  showTemp () + setRozklad();
